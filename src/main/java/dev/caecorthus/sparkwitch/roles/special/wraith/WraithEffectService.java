...
/** Applies and removes only persistent effects owned by Wraith. */
final class WraithEffectService {
    ...
    static void apply(ServerPlayerEntity player, boolean restricted) {
        ...
        RegistryEntry<StatusEffect> noCollision = noCollisionEffect();
        if (noCollision != null) {
            refresh(player, noCollision, 0);
        }
        if (restricted) {
    ...
    static void removeAll(ServerPlayerEntity player) {
        ...
        RegistryEntry<StatusEffect> noCollision = noCollisionEffect();
        if (noCollision != null) {
            player.removeStatusEffect(noCollision);
        }
    }
    ...
    private static RegistryEntry<StatusEffect> noCollisionEffect() {
        return Registries.STATUS_EFFECT.getEntry(NO_COLLISION).orElse(null);
    }
}
