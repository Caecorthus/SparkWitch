...
`sparkwitch:tofana_elixir`（托法娜仙液）是最大堆叠 1 的被动物品，没有主动右键效果。它仍只在巫女商店出售，但任何角色持有后都能使用。

当另一名仍存活且参与当前游戏的玩家对持有者发起非强制 Wathe 击杀，并且该击杀已经通过现有 `KillPlayer.BEFORE` 保护与疯魔护甲判断、尚未写入死亡状态时，托法娜仙液触发。强制击杀、环境死亡、自杀、无 killer 的脚本死亡，以及 killer 已失效时均不触发也不消耗。

“持有”以服务器在击杀瞬间的物品栏状态为准，包括副手、快捷栏和主背包；护甲槽、个人合成格、光标、已打开容器、地面掉落物和其他容器均不算。消耗顺序固定为副手优先，再按主背包/快捷栏物品格顺序查找；每次只消耗一个，即使命令生成了异常堆叠。

触发顺序固定：

1. 消耗一瓶托法娜仙液，并取消当前 Wathe 击杀。持有者不死亡、不生成尸体、不治疗，也不回滚攻击已经产生的其他状态。
2. 将反击排入服务器队列，至少等待到下一服务器 tick 再执行。
3. 执行时重新按 UUID 解析原 killer，并要求其仍在线、存活、参与当前游戏且未离开原世界。
4. 以仙液持有者作为直接 killer，发起普通、非强制 Wathe 击杀：
...
若持有者在排队后死亡或断线，已形成的反击仍保留其归属；若原 killer 失效，则丢弃反击且不返还仙液。若对方的保护、疯魔护甲或托法娜仙液挡下反击，本瓶同样不返还。双方持有仙液时允许形成逐 tick 的有限连锁，每次保护都必须先消耗一个物品。

排队反击在回合结束、世界卸载或服务器停止时清空，不持久化。巫女对 NoellesRoles 最终巫毒死亡的职业免疫保持独立并优先，因此该免疫不消耗仙液、不触发反击。

反击死亡原因翻译为中文 `仙液反噬`、英文 `Elixir Backlash`。除 Wathe 正常击杀流程已有的记录、奖励、回放与尸体外，不发送额外文字、动作栏、标题或音效反馈。
