...
/**
 * Immutable copy of Wathe task progress and generation cadence crossing the death-conversion seam.
 * 穿过死亡转化接缝的 Wathe 任务进度与生成节奏不可变快照。
 */
public final class WraithTaskSnapshot {
    ...
    static WraithTaskSnapshot capture(PlayerEntity player) {
        ...
        int timer = ((PlayerMoodComponentAccessor) component).sparkwitch$getNextTaskTimer();
    ...
    void restore(ServerPlayerEntity player) {
        ...
        ((PlayerMoodComponentAccessor) component).sparkwitch$setNextTaskTimer(nextTaskTimer);
        component.sync();
    }
}
