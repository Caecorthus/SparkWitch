package dev.caecorthus.sparkwitch.client.net.version;
...
public final class SparkWitchClientVersionHandshake {
    ...
    public static synchronized void registerClient() {
        ...
        boolean registered = ClientLoginNetworking.registerGlobalReceiver(SparkWitchVersionHandshake.VERSION_CHECK_ID,
                (client, handler, buf, callbacks) -> {
                    ...
                    boolean confirmed = confirmSparkWitchServer(serverVersion, clientVersion, "login");
                    SparkWitch.LOGGER.info(
                            "Answering SparkWitch login version query: server={}, client={}, confirmed={}.",
                            serverVersion,
                            clientVersion,
                            confirmed
                    );
        ...
        ClientPlayNetworking.registerGlobalReceiver(SparkWitchServerConfirmS2CPacket.ID, (payload, context) -> {
            String clientVersion = SparkWitchVersionHandshake.localVersion();
            if (!confirmSparkWitchServer(payload.serverVersion(), clientVersion, "play")) {
                SparkWitch.LOGGER.warn(
    ...
    private static boolean confirmSparkWitchServer(String serverVersion, String clientVersion, String stage) {
        if (!SparkWitchServerConnection.confirmCompatible(serverVersion, clientVersion)) {
            SparkWitch.LOGGER.warn(
                    "Ignoring SparkWitch {} confirmation mismatch: server={}, client={}.",
                    stage,
                    serverVersion,
                    clientVersion
            );
