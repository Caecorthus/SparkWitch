 ...
 */
public final class WitchForcedSkillState {
    private static final String FORCED_SKILL_KEY = "ForcedSkill";

    private WitchForcedSkillState() {
    }

    public static void writeToNbt(NbtCompound tag, @Nullable Identifier forcedSkillId) {
        if (forcedSkillId != null) {
            tag.putString(FORCED_SKILL_KEY, forcedSkillId.toString());
        }
    }

    public static @Nullable Identifier readFromNbt(NbtCompound tag) {
